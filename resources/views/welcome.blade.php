...
@section('content')
    ...
    <div class="space-y-16 sm:space-y-20">
        <section class="grid gap-8 py-4 lg:grid-cols-[1.08fr_0.92fr] lg:items-center">
            <div>
                <p class="text-sm font-bold uppercase tracking-normal text-brand-blue">SweepKit for private football sweepstakes</p>
                <h1 class="mt-3 max-w-4xl text-4xl font-black leading-tight text-brand-navy sm:text-5xl">
                ...
                <div class="mt-7 flex flex-wrap gap-3">
                    <a href="{{ $createSweepstakeUrl }}" class="sk-btn-green py-2.5">{{ $createSweepstakeLabel }}</a>
                    @guest
                        <a href="{{ route('login') }}" class="sk-btn-secondary py-2.5">Log in</a>
                    @else
                        <a href="{{ route('dashboard') }}" class="sk-btn-secondary py-2.5">Dashboard</a>
                    @endguest
                </div>

                <p class="mt-4 text-sm font-medium text-brand-muted">
                    Built for private groups, workplaces, clubs and friends.
                </p>
            </div>

            <div class="sk-card overflow-hidden">
                <div class="border-b border-brand-border bg-white p-5">
                    <div class="flex flex-wrap items-center justify-between gap-3">
                        <div>
                            <p class="text-sm font-semibold text-brand-blue">Example setup</p>
                            <h2 class="mt-1 text-2xl font-black text-brand-navy">Office World Cup sweepstake</h2>
                        </div>
                ...
                <div class="border-t border-brand-border bg-brand-navy p-5 text-white">
                    <p class="text-sm font-semibold text-brand-green">Balanced pots</p>
        ...
        <section>
            <div class="max-w-2xl">
                ...
                <h2 class="mt-2 text-3xl font-black text-brand-navy">From setup to draw in three steps.</h2>
            </div>

            <div class="mt-6 grid gap-4 md:grid-cols-3">
                @foreach ($steps as $step)
                    <article class="sk-card p-5">
                        <p class="text-xs font-bold uppercase tracking-normal text-brand-blue">{{ $step['label'] }}</p>
                        <h3 class="mt-3 text-lg font-bold text-brand-navy">{{ $step['title'] }}</h3>
                        <p class="mt-2 text-sm leading-6 text-brand-muted">{{ $step['body'] }}</p>
                    </article>
                @endforeach
            </div>
        </section>
        ...
        <section>
            <div class="max-w-2xl">
                ...
                <h2 class="mt-2 text-3xl font-black text-brand-navy">Everything the organiser needs in one place.</h2>
            </div>
            ...
            <div class="mt-6 grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
                @foreach ($features as $feature)
                    <article class="sk-card p-5">
                        ...
                        <p class="mt-2 text-sm leading-6 text-brand-muted">{{ $feature['body'] }}</p>
                    </article>
                @endforeach
            </div>
        </section>

        <section class="grid gap-6 lg:grid-cols-[0.95fr_1.05fr] lg:items-center">
            ...
            <div>
                ...
                <h2 class="mt-2 text-3xl font-black text-brand-navy">A fairer way to draw teams</h2>
                <div class="mt-4 space-y-4 text-base leading-7 text-brand-muted">
        ...
        <section class="grid gap-6 lg:grid-cols-[1.05fr_0.95fr] lg:items-center">
            <div>
                ...
                <h2 class="mt-2 text-3xl font-black text-brand-navy">Clear results for everyone</h2>
                <div class="mt-4 space-y-4 text-base leading-7 text-brand-muted">
                    <p>
            ...
            <div class="sk-card p-5">
                ...
                <ul class="mt-4 space-y-3 text-sm text-brand-muted">
                    <li class="flex items-center justify-between gap-4 rounded-lg border border-brand-border bg-brand-soft px-4 py-3">
                        <span class="font-semibold text-brand-navy">Japan</span>
                        <span class="sk-badge sk-badge-blue">Group draw</span>
                    </li>
                    <li class="flex items-center justify-between gap-4 rounded-lg border border-brand-border bg-white px-4 py-3">
        ...
        <section class="rounded-lg border border-brand-border bg-white p-6 shadow-sm shadow-brand-navy/5">
            <div class="max-w-3xl">
                <p class="text-sm font-bold uppercase tracking-normal text-brand-blue">Private beta</p>
                <h2 class="mt-2 text-3xl font-black text-brand-navy">Built for private sweepstakes</h2>
                <div class="mt-4 space-y-3 text-base leading-7 text-brand-muted">
                    <p>
        ...
        <section class="rounded-lg bg-brand-navy px-6 py-8 text-white sm:px-8 sm:py-10">
            <div class="flex flex-col gap-6 sm:flex-row sm:items-center sm:justify-between">
                <div class="max-w-2xl">
                    <h2 class="text-3xl font-black">Ready to run your draw?</h2>
